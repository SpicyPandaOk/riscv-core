...
initial begin;
    ...
    check_registers(5'd2, 32'd1, "xori x2, x0, 1"); //xori
    check_registers(5'd3, 32'd1, "ori x3, x0, 1"); //ori
    check_registers(5'd4, 32'd1, "andi x4, x0, 1"); //andi
    check_registers(5'd5, 32'd4, "slli x5, x0, 4"); //slli
    check_registers(5'd6, 32'd1, "srli x6, x0, 1"); //srli
    check_registers(5'd7, 32'd0, "srai x7, x0, 1"); //srai
    check_registers(5'd8, 32'd0, "slti x8, x0, 1"); //slti
    check_registers(5'd9, 32'd1, "sltiu x9, x0, 1"); //sltiu
    $display("\nStarting second set of tests: base arithmetic");
    check_registers(5'd10, 32'd1, "add x10, x0, x0"); //add
    check_registers(5'd11, 32'd0, "sub x11, x0, x0"); //sub
    check_registers(5'd12, 32'd1, "xor x12, x0, x0"); //xor
    check_registers(5'd13, 32'd1, "or x13, x0, x0"); //or
    check_registers(5'd14, 32'd1, "and x14, x0, x0"); //and
    check_registers(5'd15, 32'd2, "sll x15, x0, 2"); //sll
    check_registers(5'd16, 32'd1, "srl x16, x0, 1"); //srl
    check_registers(5'd17, 32'd0, "sra x17, x0, 1"); //sra
    check_registers(5'd18, 32'd1, "slt x18, x0, x0"); //slt
    check_registers(5'd19, 32'd0, "sltu x19, x0, x0"); //sltu
    $display("\nStarting third set of tests: memory");
    check_mem(32'd0, 32'd1, "sw x0, 0(x0)"); //sw
    check_mem(32'd1, 32'd1, "sb x0, 0(x0)"); //sb
    check_mem(32'd2, 32'd1, "sh x0, 0(x0)"); //sh  
    check_registers(5'd20, 32'd1, "lb x20, 0(x0)"); //lb
    check_registers(5'd21, 32'd1, "lh x21, 0(x0)"); //lh
    check_registers(5'd22, 32'd1, "lw x22, 0(x0)"); //lw
    $display("\nStarting fourth set of tests: branches");
    skip_instr("beq x2, x0, 8"); //beq
    check_registers(5'd2, 32'd50, "addi x2, x0, 50 (branch taken)"); //addi
    skip_instr("bne x2, x0, 8"); //bne
